import { env } from "@/env"
import { BookQueryParams, BookQueryResponse, DefaultBookQueryParams } from "@/types/books"
...
export const GoogleClient = createApi({
  ...
  endpoints: build => ({
    booksGetVolumes: build.query<BookQueryResponse, BookQueryParams>({
      query: (
        queryOptions: BookQueryParams = DefaultBookQueryParams,
      ) => {
        ...
        logger({ breakpoint: '[google.api.ts:63]/booksGetVolumes' }, {
          queryOptions,
          queryParams,
          request,
        })
