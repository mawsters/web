...
export const { appendCollection, setCollection, showPopup } = collectionSlice.actions;

// get all collections
export const initializeCollections = () => {
    return async (dispatch: Dispatch) => {
        const collections = await collectionAPI.getAll();
        console.log("initialize Collections", collections);
...
export const updateCollection = ( id: number, updatedField: object) => {
    return async (dispatch: Dispatch) => {
        // Access the current state
        // Get the current length of the collections data array        
        const collections = await collectionAPI.update({id, updatedField: updatedField});
        dispatch(setCollection(collections))
    }
}
...
export const deleteCollectionInStore = (id: number) => {
    return async (dispatch: Dispatch) => {
        ...
        console.log('====================================');
        console.log("Collections After Delete", collections);
        console.log('====================================');
        dispatch(setCollection(collections))
    }
}